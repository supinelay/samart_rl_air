...
class Gui():
    def __init__(self, env_config, init_pos):
        ...
        with open('E://ZJUT_HL\ZNLJ//blueair_new//algorithms//IL//data2traj//trjs.pkl', 'rb') as f:
            self.base_line = pickle.load(f)

        # 定义画线点存储列表
        self.line_positions = None
        self.base_line_positions_1 = list()
        self.base_line_positions_2 = list()
    ...
    def reset(self):
        ...
        self.cur_step = 0
        self.replay_step = 0
        for i in range(len(self.base_line[0])):
            base_line = self.base_line[0][i]
            self.base_line_positions_1.append((base_line[0] / self.Scaling, self.YLength - base_line[1] / self.Scaling))

        for i in range(len(self.base_line[4])):
            base_line_2 = self.base_line[4][i]
            self.base_line_positions_2.append((base_line_2[0] / self.Scaling,
                                               self.YLength - base_line_2[1] / self.Scaling))
    ...
    def _show(self, red_state, blue_state):
        ...
        if len(self.base_line_positions_1) > 1:
            pygame.draw.aalines(self.screen, (255, 0, 0), False, self.base_line_positions_1, 5)
        if len(self.base_line_positions_2) > 1:
            pygame.draw.aalines(self.screen, (255, 0, 0), False, self.base_line_positions_2, 5)

        # 绘制按钮
        self.back_button.draw(self.screen, self.button_back_img, scale=(30, 30), position=(835, 35))
        self.pause_button.draw(self.screen, self.button_paused_img, scale=(20, 30), position=(900, 35))
        self.forward_button.draw(self.screen, self.button_forward_img, scale=(30, 30), position=(965, 35))
