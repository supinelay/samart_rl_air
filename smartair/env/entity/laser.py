...
class Config:
    def __init__(self):
        ...
        self.MaxDeltaAngle = math.pi / 180 * 1       # 一个仿真步长可以走的最大角度   0.05s
        self.DTime = 2
        self.MaxAtkAngle = math.pi / 180 * 0.05       # 一半的镭射误差范围
...
class Laser(Config):
    def __init__(self,  side, state):
        ...
        self.side = side
        self.init_state = {"X": self.X, "Y": self.Y, "Angle": self.angle, "type": "laser", "locked_plane": []}
    ...
    def execute_attack(self, blue_state):
        ...
        wave_pos = np.array([self.cur_state['X'],
                             self.cur_state['Y']])
        threat_target = get_min_time(blue_state, wave_pos, self.KillingR, plane_num)
        attack_target = choice(threat_target).item()
        target = blue_state['plane_{}'.format(attack_target)]
        return target
