...
import numpy as np
from algorithms.TD3.networks import ActorNetwork, CriticNetwork
from algorithms.TD3.buffer import ReplayBuffer
...
class TD3BC(nn.Module):
    def __init__(self, alpha, beta, state_dim, action_dim, actor_fc1_dim, actor_fc2_dim,
                 critic_fc1_dim, critic_fc2_dim, ckpt_dir, bc_buffer_dir, device, gamma=0.99, tau=0.005, action_noise=0.1,
                 policy_noise=0.2, policy_noise_clip=0.5, delay_time=2, max_size=1000000,
                 batch_size=256, bc_proportion=0.3, bc_learn_period=5):
        ...
        self.bc_batch_size = int(bc_proportion*batch_size)
        self.bc_period = bc_learn_period
        self.checkpoint_dir = ckpt_dir
        self.bc_buffer_dir = bc_buffer_dir
        self.device = device
        ...
        self.bc_memory = ReplayBuffer(state_dim=state_dim, action_dim=action_dim, reward_dim=1,
    ...
    def bc_learn(self):
        states_bc, action_bc, _, _, _ = self.bc_memory.bc_sample(bc_batch_size=self.bc_batch_size)
        action_pred = self.actor(states_bc)
        loss_bc = self.bc_loss_function(action_pred, action_bc)
    ...
    def learn(self):
        if len(self.memory) < self.batch_size:           # 当 memory 中不满足一个批量时，不更新策略
            return 0.0, 0.0
        if self.update_time % self.bc_period == 0:
            self.bc_learn()
    ...
    def load_bc_buffer(self, option):
        self.memory.buffer = torch.load(os.path.join(self.bc_buffer_dir, 'BC_data_{}.pt').format(option))
